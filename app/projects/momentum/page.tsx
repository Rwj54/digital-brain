...
function getMomentumTone(score: number | null): string {
...
function getBarWidth(value: number | null, min: number, max: number): number {
...
export default function MomentumOverviewPage() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          json && "error" in json
            ? json.error
            : "Failed to load authority chart."
        );
      }
  ...
  const validMomentumPoints = useMemo(
    () =>
      chartSeries
        .map((point) => point.momentum)
        .filter((value): value is number => value !== null),
    [chartSeries]
  );

  const authorityMin = validAuthorityPoints.length
    ? Math.min(...validAuthorityPoints)
    : 0;
  const authorityMax = validAuthorityPoints.length
    ? Math.max(...validAuthorityPoints)
    : 0;
  const momentumMin = validMomentumPoints.length
    ? Math.min(...validMomentumPoints)
    : 0;
  const momentumMax = validMomentumPoints.length
    ? Math.max(...validMomentumPoints)
    : 0;
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div
              className={`mt-3 text-4xl font-black ${getMomentumTone(
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-3 text-4xl font-black">
              {formatScore(latest?.authority_score ?? null)}
            </div>
            <div className="mt-2 text-sm text-zinc-600">
              {latest?.authority_tier ?? "—"}
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Current component inputs used by the momentum model.
          </p>
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Daily chart history from the authority chart route.
          </p>
          ...
          {!chartSeries.length ? (
            ...
            <div className="mt-4 space-y-3">
              {chartSeries
                ...
                .reverse()
                .map((point) => {
                  const authorityWidth = getBarWidth(
                    point.authority,
                    authorityMin,
                    authorityMax
                  );
                  const momentumWidth = getBarWidth(
                    point.momentum,
                    momentumMin,
                    momentumMax
                  );
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="text-xs text-zinc-600">
                          Authority {formatScore(point.authority)} • Momentum{" "}
                          {formatScore(point.momentum)}
                        </div>
                      </div>
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Inspector view for validating the momentum pipeline.
          </p>
