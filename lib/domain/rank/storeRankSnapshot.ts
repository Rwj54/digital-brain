...
export async function storeRankSnapshot(input: StoreRankSnapshotInput) {
  ...
  const snapshotDate = capturedAt ?? new Date().toISOString().slice(0, 10);

  const { error } = await supabase.from("gbp_rank_snapshots").upsert(
    {
    ...
    {
      onConflict: "project_id,keyword,metro,captured_at,rank_position",
    }
  );
