...
type ProfileStructuralSignals = {
  ...
  hasPhone: boolean;
  hasWebsite: boolean;
  primaryCategoryKnown: boolean;
  additionalCategoriesKnown: boolean;
...
export type AuthorityBaselineResult = {
  executed: boolean;
...
function hasKnownNonEmptyString(value: unknown): boolean {
  return typeof value === "string" && value.trim().length > 0;
}

function isKnownFiniteNumber(value: unknown): value is number {
  return typeof value === "number" && Number.isFinite(value);
}
...
function countStoredActions(value: unknown): number {
...
function extractProfileStructuralSignals(profileRow: AnyRow): ProfileStructuralSignals {
  const additionalCategoriesRaw = profileRow["additional_categories"];
  ...
  const primaryCategoryKnown =
    Object.prototype.hasOwnProperty.call(profileRow, "primary_category") &&
    profileRow["primary_category"] !== null &&
    profileRow["primary_category"] !== undefined;

  const additionalCategoriesKnown =
    Array.isArray(additionalCategoriesRaw) ||
    (rawProvider !== null &&
      Object.prototype.hasOwnProperty.call(rawProvider, "additional_categories"));

  const additionalCategoryCount = Array.isArray(additionalCategoriesRaw)
    ? additionalCategoriesRaw.length
    : Array.isArray(rawProvider?.additional_categories)
      ? rawProvider.additional_categories.length
      : 0;

  const descriptionKnown = rawProvider
    ? Object.prototype.hasOwnProperty.call(rawProvider, "description")
    : false;
  const hoursKnown = rawProvider
    ? Object.prototype.hasOwnProperty.call(rawProvider, "hours")
    : false;
  const phoneKnown = rawProvider
    ? Object.prototype.hasOwnProperty.call(rawProvider, "phone")
    : false;
  const websiteKnown = rawProvider
    ? Object.prototype.hasOwnProperty.call(rawProvider, "website")
    : false;

  const hasPrimaryCategory = hasKnownNonEmptyString(profileRow["primary_category"]);
  const hasDescription = descriptionKnown && hasKnownNonEmptyString(rawProvider?.description);
  const hasPhone = phoneKnown && hasKnownNonEmptyString(rawProvider?.phone);
  const hasWebsite = websiteKnown && hasKnownNonEmptyString(rawProvider?.website);
  const hasHours =
    hoursKnown &&
    rawProvider?.hours !== null &&
    rawProvider?.hours !== undefined &&
    (!(typeof rawProvider?.hours === "string") || rawProvider.hours.trim().length > 0);
  ...
  return {
    ...
    hasPhone,
    hasWebsite,
    primaryCategoryKnown,
    additionalCategoriesKnown,
    descriptionKnown,
    hoursKnown,
    phoneKnown,
    websiteKnown,
...
function extractProfileActivitySignals(profileRow: AnyRow): ProfileActivitySignals {
  const posts30dKnown = isKnownFiniteNumber(profileRow["posts_30d"]);
  const photosCountKnown = isKnownFiniteNumber(profileRow["photos_count"]);
  const qaCountKnown = isKnownFiniteNumber(profileRow["qa_count"]);

  return {
    posts30d: posts30dKnown ? num(profileRow, "posts_30d", 0) : 0,
    photosCount: photosCountKnown ? num(profileRow, "photos_count", 0) : 0,
    qaCount: qaCountKnown ? num(profileRow, "qa_count", 0) : 0,
    posts30dKnown,
    photosCountKnown,
    qaCountKnown,
  };
...
function buildAuthorityActions(args: {
  ...
  if (args.posts30dKnown && args.posts30d < 4) {
    actions.push({
  ...
  if (args.photosCountKnown && args.photosCount < 25) {
    actions.push({
  ...
  if (args.qaCountKnown && args.qaCount < 3) {
    actions.push({
  ...
  if (args.primaryCategoryKnown && !args.hasPrimaryCategory) {
    actions.push({
  ...
  if (args.additionalCategoriesKnown && args.additionalCategoryCount < 2) {
    actions.push({
  ...
  if (args.descriptionKnown && !args.hasDescription) {
    actions.push({
  ...
  if (args.hoursKnown && !args.hasHours) {
    actions.push({
  ...
  if (args.phoneKnown && !args.hasPhone) {
    actions.push({
  ...
  if (args.websiteKnown && !args.hasWebsite) {
    actions.push({
...
async function loadExistingAuthorityState(params: {
  supabase: ReturnType<typeof supabaseAdmin>;
  projectId: string;
  capturedAt: string;
  version: string;
}): Promise<{
  authority: AuthorityScoreRow | null;
  actions: ProjectActionsRow | null;
}> {
  const { supabase, projectId, capturedAt, version } = params;

  const [{ data: authorityRow, error: authorityError }, { data: actionsRow, error: actionsError }] =
    await Promise.all([
      supabase
        .from("project_authority_scores")
        .select("authority_score, authority_tier, momentum_score, momentum_label, inputs, captured_at")
  ...
  if (authorityError) {
    throw new Error(`project_authority_scores same-day read error: ${authorityError.message}`);
  }

  if (actionsError) {
    throw new Error(`project_actions same-day read error: ${actionsError.message}`);
  }

  return {
    authority: (authorityRow ?? null) as AuthorityScoreRow | null,
    actions: (actionsRow ?? null) as ProjectActionsRow | null,
  };
}

export async function runAuthorityBaseline(params: {
  projectId: string;
  capturedAt: string;
  version?: string;
}): Promise<AuthorityBaselineResult> {
  const supabase = supabaseAdmin();
  const version = params.version ?? "v1.1";
  const yesterday = yesterdayDateUTC();

  const existingState = await loadExistingAuthorityState({
    supabase,
  ...
  const existingAuthorityScore = parseFiniteNumber(existingState.authority?.authority_score);
  const existingMomentumScore = parseFiniteNumber(existingState.authority?.momentum_score);
  const existingActionsCount = countStoredActions(existingState.actions?.actions_json);

  if (
    existingAuthorityScore !== null &&
    typeof existingState.authority?.authority_tier === "string" &&
    existingState.authority.authority_tier.trim() !== "" &&
    existingMomentumScore !== null &&
    typeof existingState.authority?.momentum_label === "string" &&
    existingState.authority.momentum_label.trim() !== "" &&
    existingActionsCount > 0
  ) {
    return {
      executed: true,
      authorityScore: existingAuthorityScore,
      authorityTier: existingState.authority.authority_tier,
      momentumScore: existingMomentumScore,
      momentumLabel: existingState.authority.momentum_label,
      actionsCount: existingActionsCount,
      skippedReason:
        "Authority baseline reused existing same-day authority score and actions for this project, captured date, and version.",
    };
  }
  ...
  if (profileError) {
    throw new Error(`gbp_profiles error: ${profileError.message}`);
  }
  ...
  if (!profile) {
    return {
      ...
      momentumLabel: null,
      actionsCount: 0,
      skippedReason: "GBP profile is missing, so authority baseline was skipped.",
    };
  }
  ...
  const activitySignals = extractProfileActivitySignals(profileRow);
  ...
  if (metricsError) {
    throw new Error(`gbp_competitor_metrics error: ${metricsError.message}`);
  }
  ...
  if (metricsRows.length === 0) {
    return {
      ...
      actionsCount: 0,
      skippedReason:
        "Competitor metrics are missing, so authority baseline was skipped.",
    };
  }
  ...
  const structuralSignals = extractProfileStructuralSignals(profileRow);
  ...
  const computed = computeAuthority({
    ...
    hasPrimaryCategory: structuralSignals.hasPrimaryCategory,
    additionalCategoryCount: structuralSignals.additionalCategoryCount,
    hasDescription: structuralSignals.hasDescription,
    hasHours: structuralSignals.hasHours,
    hasPhone: structuralSignals.hasPhone,
    hasWebsite: structuralSignals.hasWebsite,
  ...
  if (!validateAuthorityOutput(computed as unknown as Record<string, unknown>)) {
    throw new Error("Authority engine returned invalid output for this project.");
  }

  const { data: yRow, error: previousError } = await supabase
    .from("project_authority_scores")
  ...
  if (previousError) {
    throw new Error(`project_authority_scores read error: ${previousError.message}`);
  }
  ...
  const inputs: JsonObject = {
    ...
    gbp_activity: {
      posts_30d: activitySignals.posts30dKnown ? activitySignals.posts30d : null,
      photos_count: activitySignals.photosCountKnown ? activitySignals.photosCount : null,
      qa_count: activitySignals.qaCountKnown ? activitySignals.qaCount : null,
    },
    activity_signal_knowledge: {
      posts_30d_known: activitySignals.posts30dKnown,
      photos_count_known: activitySignals.photosCountKnown,
      qa_count_known: activitySignals.qaCountKnown,
    },
    structural_signal_knowledge: {
      primary_category_known: structuralSignals.primaryCategoryKnown,
      additional_categories_known: structuralSignals.additionalCategoriesKnown,
      description_known: structuralSignals.descriptionKnown,
      hours_known: structuralSignals.hoursKnown,
      phone_known: structuralSignals.phoneKnown,
      website_known: structuralSignals.websiteKnown,
    },
    momentum: {
  ...
  if (upsertError) {
    throw new Error(`project_authority_scores upsert error: ${upsertError.message}`);
  }
  ...
  const authorityActions = buildAuthorityActions({
    ...
    hasPhone: structuralSignals.hasPhone,
    hasWebsite: structuralSignals.hasWebsite,
    primaryCategoryKnown: structuralSignals.primaryCategoryKnown,
    additionalCategoriesKnown: structuralSignals.additionalCategoriesKnown,
    descriptionKnown: structuralSignals.descriptionKnown,
    hoursKnown: structuralSignals.hoursKnown,
    phoneKnown: structuralSignals.phoneKnown,
    websiteKnown: structuralSignals.websiteKnown,
  ...
  if (rankKeywordsError) {
    throw new Error(`project_rank_keywords error: ${rankKeywordsError.message}`);
  }
  ...
  for (const row of keywordRows) {
    ...
    try {
      ...
      throw new Error(
        `pressure action error (${keyword}): ${getErrorMessage(
          pressureError,
          "Unknown error"
        )}`
      );
  ...
  return {
    executed: true,
    authorityScore: computed.authorityScore,
    authorityTier: computed.authorityTier,
